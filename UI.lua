...
do
    ...
    local updateSlider = function()
        ...
        local sliderPosition = slider.Slider.Bar.AbsolutePosition
        local sliderSize = slider.Slider.Bar.AbsoluteSize

        local relativePosition = math.clamp((mouse.X - sliderPosition.X) / sliderSize.X, 0, 1)
        value = math.clamp(min + (max - min) * relativePosition, min, max)
